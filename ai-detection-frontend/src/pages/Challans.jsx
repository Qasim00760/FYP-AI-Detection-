...
function Challans() {
  ...
  const handleMarkPaid = async (challanNumber) => {
    setMarkingId(challanNumber); setMessage(null)
    try {
  ...
  const totalAmt = challans.filter(c => c.status === 'paid').reduce((acc, c) => acc + (parseFloat(c.amount) || 0), 0)
    ...
    <div className="bg-slate-950 text-white min-h-screen py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-slate-900">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold font-display text-white flex items-center gap-3">🎫 Challan Records</h1>
            <p className="text-slate-400 text-sm mt-1">All issued challans for helmet violations</p>
          </div>
          <div className="flex gap-3">
            <button onClick={fetchChallans} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-semibold border border-slate-700">🔄 Refresh</button>
            <button onClick={downloadCSV} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold">⬇️ Download CSV</button>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Challans',   value: total,   color: 'text-blue-400',    icon: '📄' },
            { label: 'Pending',          value: pending, color: 'text-amber-400',   icon: '⏳' },
            { label: 'Paid',             value: paid,    color: 'text-emerald-400', icon: '✅' },
            { label: 'Amount Collected', value: `Rs. ${totalAmt.toLocaleString()}`, color: 'text-indigo-400', icon: '💰' },
          ].map((s, i) => (
        ...
        {message && (
          <div className={`rounded-lg p-3 text-sm font-semibold ${message.type === 'success' ? 'bg-emerald-950/40 border border-emerald-500/30 text-emerald-300' : 'bg-rose-950/40 border border-rose-500/30 text-rose-300'}`}>
            {message.text}
          </div>
        )}
        ...
        {loading ? (
            ...
            <table className="w-full text-sm text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {challans.map((c, idx) => {
                  ...
                  return (
                    <tr key={idx} className={`transition-colors ${isPaid ? 'bg-emerald-950/10 hover:bg-emerald-950/20' : 'bg-amber-950/10 hover:bg-amber-950/20'}`}>
                      <td className="px-5 py-3.5 text-slate-500 text-xs">{idx + 1}</td>
                      <td className="px-5 py-3.5 font-mono text-xs text-yellow-400 font-bold">{c.challan_number || '—'}</td>
                      <td className="px-5 py-3.5">
                        <span className="px-3 py-1 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 font-mono text-xs">{c.vehicle_number || 'UNKNOWN'}</span>
                      </td>
                      <td className="px-5 py-3.5 text-emerald-400 font-bold">Rs. {parseFloat(c.amount || 0).toLocaleString()}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${isPaid ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                          {isPaid ? 'Paid' : 'Pending'}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        {!isPaid && (
                          <button onClick={() => handleMarkPaid(c.challan_number)} disabled={markingId === c.challan_number}
          ...
          <div className="text-center py-20 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
            ...
            <p className="text-slate-400 text-sm max-w-md mx-auto">Challans will appear here once generated from the Detection Dashboard.</p>
          </div>
        )}
      </div>
    </div>
